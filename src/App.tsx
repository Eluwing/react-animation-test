...
function App() {
  const tempCardContents = Array.from({ length: 12 }, (_, i) => (i + 1).toString());
      ...
      <div className="min-h-[600px] parallax bg-fixed bg-center bg-no-repeat bg-cover ">
        ...
        <div className="absolute grid grid-cols-2 gap-10 left-1/2 transform -translate-x-1/2">
          {tempCardContents.map((item)=>(
            <Card className="w-[200px] h-[150px] border-2 border-white bg-white text-black mt-20" contents={item} />
          ))}
        </div>
        </div>
        <div className="bg-parallax-2 min-h-[600px] parallax bg-fixed bg-center bg-no-repeat bg-cover ">
        </div>
        <div className="bg-parallax-3 min-h-[600px] parallax bg-fixed bg-center bg-no-repeat bg-cover">
        </div>
      </div>
    </div>
